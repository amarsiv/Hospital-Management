...
import numpy as np
my_conn = sqltor.connect (host='localhost', user = 'root', passwd =
'12345678', database = 'appointments')
my_conn1= sqltor.connect (host='localhost', user = 'root', passwd =
'12345678', database = 'patients')
...
def dialog1():
    ...
    if (username == 'admin' and  password == 'adminpass'):
        ...
        l1 = Label(MAIN_WINDOW, text = 'Admin Dashboard', font = ('noto
sans cjk tc', 18, 'bold'))
        ...
        def openNewWindow():
           ...
           newWindow.geometry("200x200")
               ...
               record')
     l= Label(newWindow12, text = 'succesfully added the
     l.pack(padx=15)
newWindow12=Tk()
...
APPOINTMENT PROGRAM CODE
         def appointments() :
            ...
            title = Label(newWindow,text = 'Appointment Dashboard', font
= ('noto sans cjk tc', 18, 'bold'))
...
d_id = entry3a.get()
date = entry4a.get()
j = [{'A_ID':a_id,'P_Id':p_id,'D_Id':
m = pd.DataFrame(j)
print (m)
engine =
create_engine('mysql+pymysql://root:12345678@localhost/IP1')
                    mycon=engine.connect()
                    m.to_sql('appointments', mycon, index=False,
if_exists = 'append')
               Label1a = Label(newWindow12,text = 'Appointment ID:',
font=('noto sans cjk tc', 10))
               Label1a.pack(padx=15,pady= 5)
               entry1a = Entry(newWindow12, width=50, fg = "red",
font=('helvetica', 13), bg='mint cream')
               entry1a.pack(padx=15)
               Label2a = Label(newWindow12,text = 'Patient ID: ',
font=('noto sans cjk tc', 10))
Label2a.pack(padx = 15,pady=6)
     
                              entry2a = Entry(newWindow12, width=50, fg = "red",
font=('helvetica', 13), bg='mint cream')
               entry2a.pack(padx = 15,pady=7)
               Label3a = Label(newWindow12, text = 'Doctor ID',
font=('noto sans cjk tc', 10))
               Label3a.pack(padx=15, pady = 6)
               entry3a = Entry(newWindow12, width=50, fg = "red",
font=('helvetica', 13), bg='mint cream')
               entry3a.pack(padx=15, pady=7)
               Label4a = Label(newWindow12, text = 'Date', font=('noto
sans cjk tc', 10))
               entry4a= Entry(newWindow12, width=50, fg = "red",
font=('helvetica', 13), bg='mint cream')
               Label4a.pack(padx=15, pady=6)
               entry4a.pack(padx=15, pady=7)
               btn5 = Button(newWindow12, text = 'Submit',command =
table, bg='white', font=('FreeSans', 14))
               btn5.pack(padx =5)
           CODE FOR VIEWING APPOINTMENT LIST
          def appointmentlist() :
               my_w = Tk()
...
for j in range(len(Appointments)):
    e = Entry(my_w, width=25, fg='black', font =
    e.grid(row=i, column=j)
e.insert(END, Appointments[j]) i=i+ 1
...
CODE FOR DELETING AN APPOINTMENT FROM THE LIST
      def delapp ():
          ...
          myw2.configure(bg='cyan')
                               ...
                               Labelo = Label(myw2,text = 'Appointment ID:',
font=('FreeSans', 14))
                Labelo.pack(padx=15,pady= 20)
                entry1o = Entry(myw2, width=50, fg = "red",
font=('helvetica', 13), bg='mint cream')
                ...
                def delappt ():
                        ...
                        engine3 =
create_engine('mysql+pymysql://root:12345678@localhost/IP1')
                        r_set=engine3.execute("delete FROM appointments
where A_Id like '%s';" %(u,))
                        box.showinfo('info',' Succesfullu Removed a
Patient')
                btn99 = Button(myw2, text = 'Remove', command = delappt,
bg='white', font=('FreeSans', 14))
                btn99.pack(padx=5, pady = 20)
CODE FOR GRAPHICAL ANALYSIS OF APPOINTMENTS
            def agraph():
                mycon = sqltor.connect(host='localhost', user='root',
passwd = '12345678', database = 'IP1')
                df3 = pd.read_sql("Select Date,Count(Date) from
appointments group by Date;", mycon)
 'cyan')
root = Tk()
...
df1.plot(kind='bar', legend=True, ax=ax1, color =
ax1.set_title('Occupancy on a Date')
root.mainloop()
            btn6 = Button(newWindow, text = 'See Appointments',command =
appointmentlist, bg='white', font=('FreeSans', 14))
            btn6.pack(padx =5, pady =20)
            btn69 = Button(newWindow, text = 'Add an Appointment',
command = app, bg='white', font=('FreeSans', 14))
btn69.pack(padx=5, pady=20)
     
                           btn79 = Button(newWindow, text = 'Remove an Appointment',
command = delapp, bg='white', font=('FreeSans', 14))
            btn79.pack(padx=5, pady = 20)
            bt01 = Button(newWindow, text = 'Reports', command = agraph,
bg='white', font=('FreeSans', 14))
            bt01.pack(padx=35, pady=20)
              CODE FOR PATIENTS DASHBOARD
        def Patients():
            newWindow1=Tk()
            newWindow1.title("Patients")
            newWindow1.geometry("600x500+300+100")
            newWindow1.configure(bg='Orange')
            title = Label(newWindow1,text = 'Patient Dashboard', font =
('noto sans cjk tc', 18, 'bold'))
            title.pack(padx = 5, pady = 20)
            ...
            def pati():
                def ptable():
                    ...
                    n = [{'P_Name':p_name,'P_Id':p_id,'Need':
need,'Symptoms':symptoms}]
                    m1 = pd.DataFrame(n)
print (m1)
                    engine =
create_engine('mysql+pymysql://root:12345678@localhost/IP1')
  = 'append')
    mycon=engine.connect()
    m1.to_sql('patients', mycon, index=False, if_exists
newWindow122=Tk()
...
root.mainloop()
CODE FOR DELETING PATIENT ENTRY FROM LIST
                     def delpatt ():
                            u = entry1o.get()
                            engine3 =
create_engine('mysql+pymysql://root:12345678@localhost/IP1')
                            r_set=engine3.execute("delete FROM patients
                            box.showinfo('info',' Succesfullu Removed a
                    btn99.pack(padx=5, pady = 20)
CODE FOR PATIENT GRAPHICAL ANALYSIS
            def pgraph():
                mycon = sqltor.connect(host='localhost', user='root',
passwd = '12345678', database = 'IP1')
                df3 = pd.read_sql("Select Need,Count(Need) from patients
group by Need;", mycon)
where P_Id like %s;" %(u,))
Patient')
delpatt)
btn99 = Button(myw1, text = 'Remove', command =
             btn8= Button(newWindow1, text = 'See Patients ', command =
patientslist, bg='white', font=('FreeSans', 14))
            btn8.pack(padx=5, pady=20)
            btn69 = Button(newWindow1, text = 'Add a Patient', command =
pati, bg='white', font=('FreeSans', 14))
            btn69.pack(padx=5, pady=20)
            btn799 = Button(newWindow1, text = 'Remove a Patient',
command = delpat, bg='white', font=('FreeSans', 14))
btn799.pack(padx=5, pady = 20)
     
                           bt01 = Button(newWindow1, text = 'Report', command = pgraph,
bg='white', font=('FreeSans', 14))
            bt01.pack(padx=35, pady=20)
              CODE FOR DOCTORS DASHBOARD
        def Doctors():
            newWindow2=Tk()
            newWindow2.title("Doctors")
            newWindow2.geometry("600x500+300+100")
            newWindow2.configure(bg='Yellow')
            title = Label(newWindow2,text = 'Doctor Dashboard', font =
('noto sans cjk tc', 18, 'bold'))
            title.pack(padx = 5, pady = 20)
            title.configure(bg = 'Coral')
            CODE FOR ADDING DOCTORS ENTRIES
            def docc():
                def dtable():
                    d_name = entry1c.get()
                    d_id = entry2c.get()
                    ...
                    Fees = entry5c.get()
                    n1 =
[{'D_ID':d_id,'Doctor_Name':d_name,'Speciality':
speciality,'Timings':timing, 'Fees' : Fees}]
                    m1 = pd.DataFrame(n1)
print (m1)
                    engine =
create_engine('mysql+pymysql://root:12345678@localhost/IP1')
  'append')
    mycon=engine.connect()
    m1.to_sql('doctors', mycon, index=False, if_exists =
newWindow121=Tk()
...
Label2c.pack(padx = 15,pady=6)
                ...
                entry5c.pack()
                btn11 = Button(newWindow121, text = 'Submit',command =
dtable, bg='white', font=('FreeSans', 14))
                btn11.pack(padx =5)
                newWindow121.mainloop()
             CODE FOR VIEWING DOCTORS LIST
            def doclist() :
                    my_w = Tk()
...
my_w.mainloop()
                ...
                ""')
CODE FOR DELETING DOCTOR ENTRY FROM LIST
  def deldoc ():
          myw = Tk()
          ...
          def deldoct ():
                  u = entry1o.get()
                            engine3 =
create_engine('mysql+pymysql://root:12345678@localhost/IP1')
                            r_set=engine3.execute("delete FROM doctors
where D_Id like %s;" %(u,))
                    btn991 = Button(myw, text = 'Remove', command =
deldoct, bg='white', font=('FreeSans', 14))
                    btn991.pack(padx=5, pady = 20)
CODE FOR DOCTORS GRAPHICAL ANALYSIS
            def dgraph():
                mycon = sqltor.connect(host='localhost', user='root',
passwd = '12345678', database = 'IP1')
                df3 = pd.read_sql("Select Doctor_Name, Fees from doctors
group by Fees;", mycon)
                root = Tk()
 'yellow')
root.title('Analysis and Report')
...
ax3.scatter(df3['Doctor_Name'],df3['Fees'], color =
...
root.mainloop()
                           ...
                           btn8= Button(newWindow2, text = 'See Doctors', command =
doclist, bg='white', font=('FreeSans', 14))
            btn8.pack(padx=5, pady=20)
            btn019= Button(newWindow2, text = 'Add a Doctor', command =
docc, bg='white', font=('FreeSans', 14))
            btn019.pack(padx=35, pady=20)
            btn7999 = Button(newWindow2, text = 'Remove a Doctor',
command = deldoc, bg='white', font=('FreeSans', 14))
            btn7999.pack(padx=5, pady = 20)
            bt01 = Button(newWindow2, text = 'Report', command = dgraph,
bg='white', font=('FreeSans', 14))
            bt01.pack(padx=35, pady=20)
        btn1 = Button(MAIN_WINDOW, text = 'Appointments Portal',command
= appointments, bg='sky blue', font = ('helvetica', 16))
        btn1.pack(padx=20, pady=20)
        btn2 = Button(MAIN_WINDOW, text = 'Patients Portal',command =
Patients, bg='sky blue', font = ('helvetica', 16))
        btn2.pack(padx=20, pady=20)
        btn3 = Button(MAIN_WINDOW, text = 'Doctors Portal',command =
Doctors, bg='sky blue', font = ('helvetica', 16))
        btn3.pack(padx=20, pady=20)
        newWindow.mainloop()
        MAIN_WINDOW. mainloop()
FORMATTING OF THE MAIN WELCOME WINDOW
window = Tk()
...
Label221=Label(window, text='HOSPITAL MANAGEMENT SYSTEM', font = ('noto
sans cjk tc', 18, 'bold'), )
Label221.pack(padx=20, pady=6)
Label221.configure(bg = 'papaya whip')
Label1 = Label(window,text = 'Username:', font = ('MS sans serif', 18,
'bold'))
Label1.pack(padx=15,pady= 5)
Label1.configure(bg = 'light cyan')
entry1 = Entry(window, font=('helvetica', 20))
entry1.pack(padx=15, pady=5)
Label2 = Label(window,text = 'Password: ', font = ('MS sans serif', 18,
'bold'))
Label2.pack(padx = 15,pady=6)
Label2.configure(bg = 'light cyan')
entry2 = Entry(window, show = '•', font=('helvetica', 20))
entry2.pack(padx = 15,pady=7)
btn = Button(frame, text = 'Check Login',command = dialog1, bg =
'white', font = ('helvetica', 18, 'bold'))
